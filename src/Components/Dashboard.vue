                                        ...
                                        <table class="table table-striped" v-if="items.items">
                                            ...
                                            <tbody>
                                                <tr v-for="item in items.items" :key="item.id">
                                                    ...
                                                    <td>{{ showString(item.title, 15) }}</td>
                                                    <td>{{ showString(item.text, 20) }}</td>
                                                    <td>{{ showString(nameCategoryByID(item.categoryID), 20) }}</td>
                                        ...
                                        <table class="table table-striped" v-if="categories.items">
                                            ...
                                            <tbody>
                                                <tr v-for="category in categories.items" :key="category.id">
                                                    ...
                                                    <td>{{ category.name }}</td>
                                                    <td>{{ showString(category.image, 20) }}</td>
                                                    <td>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <form>
                                        ...
                                        <div class="form-group">
                                            <label>Slug</label>
                                            <input type="text" v-model="item.slug" placeholder="Slug" class="form-control" :class="{ 'is-invalid': item.submitted && !item.slug }">
                                        </div>
                                    </form>
                                </div>
                                <div class="modal-footer">
...
export default {
    ...
    methods: {
        nameCategoryByID(id){
            return this.categories.items.find(cat => cat.id == id).name
        },
        onFileChanged (event) {
        ...
        updateItem: function () {
            ...
            if (this.item.id) {
                ...
                const { id, name, text, date, slug, categoryID } = this.item;
                const { dispatch } = this.$store;
                
                if (name && text && date && slug && categoryID) {
                    dispatch('items/update', { id, name, text, date, slug, categoryID });
        ...
        createItem(){ 
            this.category.submitted = true;
            const { name, text, date, slug, categoryID } = this.item;
            const { dispatch } = this.$store;
            
            if (name && text && date && slug && categoryID) {
                dispatch('items/create', { name, text, date, slug, categoryID });
                $('#modalItem').modal('toggle');
            }
        },
        createCategory(){
        ...
        clearModal(){
            ...
            this.item = {name: '', text: '', date: '', slug: '', categoryID: '', submitted: false, new: true};
            this.fileName = '';
        }
    },
    created () {
